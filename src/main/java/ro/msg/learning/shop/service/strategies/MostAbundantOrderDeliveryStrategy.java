...
public class MostAbundantOrderDeliveryStrategy implements OrderDeliveryStrategy {
  ...
  public List<StockToTake> getListOfStocksToBeFound(OrderRequestDto orderRequestDto)
    ...
    List<StockToTake> resultStocks =
        orderRequestDto.getOrderedItems().stream()
            .map(
                orderedItem -> {
  ...
  private StockToTake createStockToTake(
      ProductIdWithQuantityDto orderedItem, Stock theSelectedStock) {
  ...
  private Optional<Stock> getStockForOrderedItem(ProductIdWithQuantityDto orderedItem) {
    return stockRepository.findTopByProductAndQuantityGreaterThanEqualOrderByQuantityDesc(
        Product.builder().id(orderedItem.getProductId()).build(), orderedItem.getQuantity());
  }
}
